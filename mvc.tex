...
\minor{MH: $\uparrow$ use spell check, pls. There are typos. One unusual:
``patter'' $\to$ ``pattern''\\
also ``to understand purposes''}

\section{Model-View-Controller}

\minor{MH: $\downarrow$ ``in Object Oriented Programming'' -- capitalized
...
\minor{MH: $\downarrow$ ``design idiom'' $\to$ ``design pattern''\ldots
Watch also the correct quotation marks `` and ''}
...
\minor{MH: $\uparrow$ use formal language without abbreviated expressions
such as ``let's''}
...
\assignment{MH: $\uparrow$ do we us PosgreSQL?}
\assignment{MH: $\uparrow$ an example of how the logic is split into model,
view and controller in some trivial sample application would help here
$\uparrow$ and here $\downarrow$}
...
\minor{MH: $\uparrow$ views does not what? Plush watch singular/plural
here\ldots}
...
\assignment{MH: $\uparrow$ go just a little bit further and say \emph{why}
is it a problem}
...
\minor{MH: $\uparrow$ by a blog article by \emph{author} [\emph{cite}] --
i.e.\ omit the title in the text, it's in the reference list below\ldots}

\assignment{MH: $\uparrow$ you need to explain
HMVC better, in more details.}

\minor{MH: the term ``triad'' appears out of the blue, it's not used above
when we explained MVC}
...
\minor{MH: $\uparrow$ Couorses 2 was not mentioned before, it's not
...
\minor{MH: $\uparrow$ simplify this to ``For more details see...''}
