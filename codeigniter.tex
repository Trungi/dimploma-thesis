...
\minor{MH: $\downarrow$ use proper dash ``--''\\
       ``it's'' $\to$ ``its''}
...
\minor{MH: $\uparrow$ it's not needed to mention this here, really}
...
\assignment{MH: $\uparrow$ and why this selection exactly?}
...
\minor{MH: $\uparrow$ improve English, e.g.\ ``This allows\ldots''}
\assignment{MH: what are routes and what cache? This is not clear
here\ldots You need to write more systematically}
...
\minor{MH: $\uparrow$ check the articles (here but also elsewhere): ``The
purpose'', ``a HTTP request'', ``the router'', ``the correct operation'',\ldots}
\assignment{MH: $\uparrow$ again, what router, what is the normal
operation? what cache file? What submitted data? This is all very unclear
to the reader yet}
...
\assignment{MH: $\uparrow$ now here a few things are finally properly
explained. The problem above was that you were referring to things as if
the reader should already be aware of them\ldots}
\minor{MH: $\uparrow$ English must be improved also here}
\minor{MH: $\uparrow$ notice the mismatch between the code font in the code
listing, and the code font in the text\ldots This is not good, this should
be synced}

\section{Configuration and loading}

\assignment{MH: $\downarrow$ why CI uses them? Why we have defined them?
(Explain this before you go into the details)} 
...
\minor{MH: $\downarrow$ This is a minor thing, but two lines above you said
that we now know everything needed and now you say we must first explain
active records -- you're contradicting yourself}
...
\assignment{MH: $\uparrow$ now you didn't actually explained how active
records work, you said that it's similar to other ORM mappings. But this is
not enough, especially after you said that you need to explain it\ldots}

\minor{MH: $\uparrow$ the cite above cannot stand between two sentences}
...
\minor{MH: $\uparrow$ capitalize: Listing~2.3, Figure~3.4, Table~2.5, etc.}
...
\assignment{MH: $\uparrow$ again, put some explanation before the listing}
...
\minor{MH: $\uparrow$ Improve English: ``we made a call to our model''}
