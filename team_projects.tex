...
\assignment{MH: it would be nice to have a workflow diagram for team
assignments, similar to that we have for regular assignments. This new
diagram would include additional steps such as team formation, team review,
etc. Throughout the chapter you can then refer to the diagram}

\minor{MH: please use spell check -- there are \emph{many} typos}
...
\minor{MH: $\uparrow$ Modern Approaches are now called Web Design
Technology and Methodology}

\minor{MH: $\uparrow$ ``it's'' $\to$ ``its''}
...
\minor{MH: $\uparrow$ Drop\v{c}ov\'{a} }
...
\assignment{MH: $\uparrow$ ``teaching process'' $\to$ ``process'' -- makes more sense
here, as strictly said, this is not our whole teaching process}
...
\minor{MH: $\uparrow$ ``Courses 2 Learning Management System'' $\to$
``Courses 2'' or ``Courses 2 LMS'' -- after you have introduced this
somewhere before it's much better not to repeat the whole capitalized term,
it's to clumsy to read}

\subsection{Representation}

\subsubsection{Database}

\minor{MH: $\downarrow$ ``in the database'' and ``in Figure XY'' BTW why
the only figure in the Appendix is numbered 4? BTW maybe it's better to
detail the color key of the figure in the Appendix, near the actual figure.
Or if it's the only thing in the Appendix, maybe you could put it here on a
separate page...}
...
\minor{MH: $\downarrow$ resolve the line overflows (mind LaTeX's warnings)}
...
\minor{MH: $\uparrow$ fix ``As we minimal database model''}
...
\minor{MH: $\uparrow$ ``based on the minimum and the maximum number of''}
...
\begin{figure}[h]
    ...
    \label{team_forming}
    \assignment{MH: please edit the screenshot, so that is does not have
    students like User 123... Maybe use some fake English names?}
\end{figure}
...
\assignment{MH: $\uparrow$ try to explain aa bit, why there is no option
to kick a user from the team, and also why students can't jin teams after
the forming deadline\ldots Just 1--2 sentences. If it helps, ask Nika about
this}
...
\minor{MH: $\uparrow$ please rewrite ``this solution'' somehow, it's not
apparent to the reader what we meen by it here}
\assignment{MH: $\uparrow$ let's explain this briefly instead balmiing it
on ``research purposes'' -- the reason is that we want to allow the teams
to split their work as the need, including the case when one member is
tasked with profiding all feedback}
...
\overview{MH: $\uparrow$ this is a very nice conclusion, very well said!}
...
\minor{MH: $\uparrow$ ``questions usually asks'' $\to$ ``questions usually
ask''}
\assignment{MH: $\uparrow$ mention also that the questions are fully
configurable, as supported by our data model and the application}
...
\assignment{MH: $\uparrow$ here you need to explain the mathematics how the
evaluation divides 100\% per student between the team members, and that
this is a very intuitive way to do this. I believe Nika also has some
citation from where this was taken...}
...
\assignment{MH: $\uparrow$ this is very hard to understand, I'm not sure
what you actually wanted to say... please try to rewrite this somehow}
...
\assignment{MH: $\uparrow$ The submission page is described in some
appropriate place before this? It's not very systematic to explain some of
the details related to this here for the first time. It would make more
sense to have this together somewhere before, and here to only refer to it
-- and to explain how there reviews are displayed on it...}
...
\assignment{MH: $\uparrow$ be careful, because the reviewers names are of
course not showed to the students... So make it apparent that you group by
team mebers who are being reviewed...}
...
\minor{MH: $\uparrow$ First part of teachers \emph{what}? Asi chyba nejake
slovo...}
...
\minor{MH: $\uparrow$ in English texts we avoid saying ``he'' and ``his''
of unknown person for political correctness. You may use ``she'' and
``her'' or plural pronouns (ask Marry about this)}
\assignment{MH: $\uparrow$ }
...
\assignment{MH: $\uparrow$ the form serves for \emph{teachers} to give
feedback to the srudent...}
...
\assignment{MH: $\uparrow$ tu to treba nalisat nejak jemnejsie, ze to
planujeme do buducna zlepsit, ponat to ako vylepsenie nie taku kritiku...}
